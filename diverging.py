...
for n in np.arange(2, 16.5, .5):
    ...
    sc1 = ax1.pcolormesh(Mdon_RLO, P_RLO, color_mbh, cmap = 'Spectral_r', norm=norm)
    cn1 = ax1.contour(Mdon_RLO, P_RLO, color_mbh, colors='k', levels=(-2, -1, 0, 1, 2), linestyles='solid')
    ax1.clabel(cn1, inline=1, fontsize=10)
    ...
    sc2 = ax2.pcolormesh(Mdon_RLO, P_RLO, color_q, cmap='Spectral_r', norm=norm)
    cn2 = ax2.contour(Mdon_RLO, P_RLO, color_q, colors='k', levels=(-2, -1, 0, 1, 2), linestyles='solid')
    ax2.clabel(cn2, inline=1, fontsize=10)
    ...
    plt.savefig('div_plots/fig' + str(n) + '.png', bbox_inches='tight')
    print('plotted ' + str(n))
    plt.close()
